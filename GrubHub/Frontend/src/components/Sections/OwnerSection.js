...
import { getOwnerSection } from '../../queries/queries'
...
class OwnerSection extends Component {
    ...
    componentDidMount() {
        ...
        this.props.client.query({
            query: getOwnerSection,
            // this.props.getOwnerProfile({
            variables: {
                ownername: this.state.emailcookie
            }
        }).then(response => {
            console.log("get ownersection", response);
            this.setState({
                sections: response.data.getOwnerSection,
            })
        }).catch(e => {
            console.log("error", e);
            this.setState({
...
export default withApollo(OwnerSection);